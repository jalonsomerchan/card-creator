<script lang="ts">
  ...
  const sizePresets = [
    { id: 'poker', name: 'Póker', width: 63, height: 88, note: 'Cartas de mesa habituales' },
    { id: 'bridge', name: 'Bridge', width: 57, height: 89, note: 'Más estrechas' },
    { id: 'tarot', name: 'Tarot', width: 70, height: 120, note: 'Grandes y verticales' },
    { id: 'mini-euro', name: 'Mini euro', width: 44, height: 68, note: 'Cartas pequeñas' },
    { id: 'square', name: 'Cuadrada', width: 70, height: 70, note: 'Tokens o losetas' },
    { id: 'business', name: 'Tarjeta', width: 85, height: 55, note: 'Formato horizontal' },
    { id: 'custom', name: 'Personalizado', width: 63, height: 88, note: 'Define ancho y alto' },
  ]

  const styleOptions: { id: CardStyle; name: string; hint: string }[] = [
    { id: 'fantasy', name: 'Fantasía', hint: 'Marco decorativo' },
    { id: 'minimal', name: 'Minimalista', hint: 'Limpio y editorial' },
    { id: 'classic', name: 'Clásico', hint: 'Borde grueso' },
    { id: 'prototype', name: 'Prototipo', hint: 'Boceto rápido' },
    { id: 'neon', name: 'Neón', hint: 'Alto contraste' },
    { id: 'parchment', name: 'Pergamino', hint: 'Aventura y rol' },
    { id: 'tarot', name: 'Tarot', hint: 'Místico vertical' },
    { id: 'sci-fi', name: 'Ciencia ficción', hint: 'Panel técnico' },
    { id: 'noir', name: 'Noir', hint: 'Oscuro y sobrio' },
    { id: 'kids', name: 'Infantil', hint: 'Redondeado y alegre' },
  ]
  ...
  const emptyCard = (): Card => ({
    ...
    description: 'Haz clic en estos textos para editarlos directamente sobre la carta.',
    quantity: 1,
    imageDataUrl: '',
    cornerLabels: ['1', '★', '', ''],
  ...
  const starterDeck = (): Deck => ({
    ...
    description: 'Edita textos directamente sobre la carta, elige tamaños reales y genera un PDF imprimible.',
    layout: {
      preset: 'poker',
      cardWidth: 63,
      cardHeight: 88,
  ...
  function patchCard(card: Card, changes: Partial<Card>, message = 'Carta actualizada.') {
  ...
  function applyPreset(id: string) {
    ...
    persist('Tamaño de carta actualizado.')
  }
  ...
  function setCornerVisible(card: Card, index: CornerIndex, visible: boolean) {
    card.cornerVisible[index] = visible
    deck.cards = [...deck.cards]
    persist('Esquinas actualizadas.')
  }
  ...
  function drawPdfCard(page: PDFPage, card: Card, x: number, y: number, width: number, height: number, regularFont: any, boldFont: any, image: any) {
    ...
    if (deck.layout.showDescription) wrapText(card.description, 35, 5).forEach((line, index) => page.drawText(line, { x: x + pad, y: y + height * 0.2 - index * 9, size: 7, font: regularFont, color: rgb(text.r, text.g, text.b), maxWidth: width - pad * 2 }))
    if (deck.layout.showStats) card.stats.slice(0, 3).forEach((stat, index) => page.drawText(`${safeText(stat.label, 8)} ${safeText(stat.value, 5)}`, { x: x + pad + index * ((width - pad * 2) / 3), y: y + pad, size: 6.5, font: boldFont, color: rgb(accent.r, accent.g, accent.b), maxWidth: (width - pad * 2) / 3 - 2 }))
  ...
  <section class="container-wide visual-workspace" aria-label="Editor de cartas">
        ...
        <div class="preset-grid">
          {#each sizePresets as preset}
            <button class:active={deck.layout.preset === preset.id} class="preset-card" type="button" on:click={() => applyPreset(preset.id)}>
              <strong>{preset.name}</strong><span>{preset.width} × {preset.height} mm</span><small>{preset.note}</small>
            </button>
          {/each}
        </div>
      ...
      <details class="settings-group" open>
        ...
        <div class="style-grid">
          {#each styleOptions as style}
            <button class:active={deck.layout.style === style.id} class={`style-choice style-${style.id}`} type="button" on:click={() => { deck.layout.style = style.id; persist('Estilo actualizado.') }}>
              <strong>{style.name}</strong><small>{style.hint}</small>
            </button>
          {/each}
        </div>
        <div class="form-grid two-cols">
      ...
      <div class="visual-card-grid" aria-label="Cartas del mazo">
        {#each deck.cards as card, index (card.id)}
          <article class:selected={selectedCardId === card.id} class:drag-over={overIndex === index && dragIndex !== index} class="deck-tile" draggable="true" on:dragstart={(event) => handleDragStart(index, event)} on:dragover={(event) => { event.preventDefault(); overIndex = index }} on:dragleave={() => (overIndex = null)} on:drop={(event) => handleDrop(index, event)} on:dragend={() => { dragIndex = null; overIndex = null }} on:click={() => (selectedCardId = card.id)}>
            ...
            <div class={`tile-thumb style-${deck.layout.style}`} style={`--accent-card:${deck.layout.accentColor}; background:${deck.layout.backgroundColor}; color:${deck.layout.textColor}; border-color:${deck.layout.borderColor}`}>
              {#if card.imageDataUrl && deck.layout.imageMode !== 'none'}<img src={card.imageDataUrl} alt="" />{:else}<span>{card.cornerLabels.find((_, cornerIndex) => card.cornerVisible[cornerIndex]) || '★'}</span>{/if}
    ...
    <aside class="panel editor-side">
      {#if selectedCard}
        ...
        <div class="print-card-wrap">
          <div class={`print-card style-${deck.layout.style}`} style={`aspect-ratio:${deck.layout.cardWidth}/${deck.layout.cardHeight}; background:${deck.layout.backgroundColor}; color:${deck.layout.textColor}; border-color:${deck.layout.borderColor}; --accent-card:${deck.layout.accentColor}`}>
            {#each selectedCard.cornerLabels as label, index}
              {#if selectedCard.cornerVisible[index]}
                <input class={`corner corner-${index}`} maxlength="4" aria-label={cornerNames[index]} bind:value={selectedCard.cornerLabels[index]} on:change={() => persist('Esquina actualizada.')} />
              {/if}
            {/each}

            {#if deck.layout.imageMode !== 'none'}
              <label class:full-image={deck.layout.imageMode === 'full'} class="card-image inline-image-editor" title="Cambiar imagen">
                {#if selectedCard.imageDataUrl}<img src={selectedCard.imageDataUrl} alt={`Imagen de ${selectedCard.title}`} />{:else}<span>Haz clic para añadir imagen</span>{/if}
                <input type="file" accept="image/*" on:change={(event) => handleImageUpload(event, selectedCard)} />
              </label>
            {/if}

            <div class="card-content inline-card-content">
              <input class="inline-title" aria-label="Título" bind:value={selectedCard.title} on:change={() => persist('Título actualizado.')} />
              <input class="inline-subtitle" aria-label="Subtítulo" bind:value={selectedCard.subtitle} on:change={() => persist('Subtítulo actualizado.')} />
              {#if deck.layout.showDescription}<textarea class="inline-description" aria-label="Descripción" bind:value={selectedCard.description} on:change={() => persist('Descripción actualizada.')}></textarea>{/if}
            </div>
        ...
        <details class="settings-group" open>
          <summary>Contenido de la carta</summary>
        ...
        <details class="settings-group" open>
          ...
          <div class="corner-visibility-grid">
            {#each selectedCard.cornerLabels as label, index}
              <label class="corner-toggle"><input type="checkbox" checked={selectedCard.cornerVisible[index]} on:change={(event) => setCornerVisible(selectedCard, index as CornerIndex, (event.currentTarget as HTMLInputElement).checked)} /><span>{cornerNames[index]}</span><input class="input" maxlength="4" bind:value={selectedCard.cornerLabels[index]} on:change={() => persist('Esquina actualizada.')} /></label>
            {/each}
          </div>
        </details>
        ...
        <details class="settings-group">
          ...
          <div class="subsection-header"><h3>Stats</h3><button class="btn btn-secondary compact" type="button" on:click={() => addStat(selectedCard)}>Añadir</button></div>
          <div class="stats-editor">{#each selectedCard.stats as stat, index}<div class="stat-row"><input class="input" aria-label="Nombre" bind:value={stat.label} on:change={() => persist('Estadística actualizada.')} /><input class="input" aria-label="Valor" bind:value={stat.value} on:change={() => persist('Estadística actualizada.')} /><button class="btn btn-ghost compact" type="button" on:click={() => removeStat(selectedCard, index)}>Quitar</button></div>{/each}</div>
        </details>
      {/if}
    </aside>
  </section>
...
<footer class="site-footer"><div class="container-wide footer-inner"><p><strong>Card Creator</strong> · editor visual de cartas imprimibles.</p><p>Textos e imagen editables directamente sobre la carta.</p></div></footer>
