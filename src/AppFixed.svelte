<script lang="ts">
  ...
  const STORAGE_KEY = 'card-creator-deck-v2'
  const OLD_STORAGE_KEY = 'card-creator-deck-v1'
  ...
  async function generatePdf() {
    ...
    try {
      ...
      console.error(error)
      statusMessage = 'No se pudo generar el PDF. He evitado símbolos no compatibles, pero revisa imágenes o datos raros.'
    } finally {
  ...
  function drawPdfCard(page: PDFPage, card: Card, x: number, y: number, width: number, height: number, regularFont: any, boldFont: any, image: any) {
    ...
    page.drawText(pdfText(card.title, 24), { x: x + pad, y: titleY, size: 11, font: boldFont, color: rgb(text.r, text.g, text.b), maxWidth: width - pad * 2 })
    if (card.subtitle) page.drawText(pdfText(card.subtitle, 32), { x: x + pad, y: titleY - 12, size: 7, font: regularFont, color: rgb(accent.r, accent.g, accent.b), maxWidth: width - pad * 2 })
    if (deck.layout.showDescription) wrapText(pdfText(card.description, 220), 35, 5).forEach((line, index) => page.drawText(line, { x: x + pad, y: y + height * 0.2 - index * 9, size: 7, font: regularFont, color: rgb(text.r, text.g, text.b), maxWidth: width - pad * 2 }))
    if (deck.layout.showStats) card.stats.slice(0, 3).forEach((stat, index) => page.drawText(`${pdfText(stat.label, 8)} ${pdfText(stat.value, 5)}`, { x: x + pad + index * ((width - pad * 2) / 3), y: y + pad, size: 6.5, font: boldFont, color: rgb(accent.r, accent.g, accent.b), maxWidth: (width - pad * 2) / 3 - 2 }))
...
<svelte:head>
  ...
  <meta name="description" content="Diseña cartas editando directamente sobre la carta, elige tamaños reales, estilos y genera PDF imprimible." />
</svelte:head>
...
<main id="main">
  <section class="hero-section compact-hero">
    <div class="container-wide hero-grid">
      <div class="hero-copy">
        <span class="eyebrow">Editor directo sobre la carta</span>
        <h1>Edita tu mazo como una mesa real</h1>
        <p>Haz clic en textos e imagen dentro de la carta para cambiarlos, ordena arrastrando y ajusta tamaño, esquinas y estilos.</p>
      ...
      <div class="hero-card">
        ...
        <span>{deck.layout.cardWidth} × {deck.layout.cardHeight} mm · {styleOptions.find((style) => style.id === deck.layout.style)?.name}</span>
      </div>
    </div>
  </section>
  ...
  <section class="container-wide visual-workspace" aria-label="Editor de cartas">
    <aside class="panel deck-panel">
      ...
      <details class="settings-group" open>
        <summary>Generar PDF</summary>
        <div class="generator-box">
          <strong>{totalCards} cartas · {pdfPages} página(s)</strong>
          <span>{grid.columns} columnas × {grid.rows} filas por página</span>
          <button class="btn btn-primary" type="button" on:click={generatePdf} disabled={isGeneratingPdf}>{isGeneratingPdf ? 'Generando...' : 'Generar PDF para imprimir'}</button>
          {#if pdfUrl}<a class="btn btn-secondary" href={pdfUrl} download={`${slugify(deck.name)}.pdf`}>Descargar último PDF</a>{/if}
        </div>
      </details>

      <details class="settings-group" open>
        <summary>Nombre y descripción</summary>
        <div class="form-grid">
      ...
      <details class="settings-group" open>
        <summary>Tamaños predefinidos</summary>
        <div class="preset-grid">
          {#each sizePresets as preset}
            <button class:active={deck.layout.preset === preset.id} class="preset-card" type="button" on:click={() => applyPreset(preset.id)}>
              <strong>{preset.name}</strong><span>{preset.width} × {preset.height} mm</span>
            </button>
          {/each}
        </div>
        <div class="form-grid two-cols">
          <label><span class="label">Ancho mm</span><input class="input" type="number" min="30" max="160" bind:value={deck.layout.cardWidth} on:change={() => { deck.layout.preset = 'custom'; persist('Tamaño personalizado.')} } /></label>
          <label><span class="label">Alto mm</span><input class="input" type="number" min="30" max="220" bind:value={deck.layout.cardHeight} on:change={() => { deck.layout.preset = 'custom'; persist('Tamaño personalizado.')} } /></label>
      ...
      <details class="settings-group" open>
        ...
        <div class="style-grid">
          {#each styleOptions as style}
            <button class:active={deck.layout.style === style.id} class={`style-choice style-${style.id}`} type="button" on:click={() => { deck.layout.style = style.id; persist('Estilo actualizado.') }}><strong>{style.name}</strong></button>
          {/each}
        </div>
        <div class="form-grid two-cols">
    ...
    <section class="panel visual-panel">
      ...
      <div class="visual-card-grid" aria-label="Cartas del mazo">
        {#each deck.cards as card, index (card.id)}
          <article class:selected={selectedCardId === card.id} class:drag-over={overIndex === index && dragIndex !== index} class="deck-tile" draggable="true" on:dragstart={(event) => handleDragStart(index, event)} on:dragover={(event) => { event.preventDefault(); overIndex = index }} on:dragleave={() => (overIndex = null)} on:drop={(event) => handleDrop(index, event)} on:dragend={() => { dragIndex = null; overIndex = null }} on:click={() => (selectedCardId = card.id)}>
            <div class="tile-drag" title="Arrastrar para ordenar">⋮⋮</div>
            <div class={`mini-card-preview style-${deck.layout.style}`} style={`aspect-ratio:${deck.layout.cardWidth}/${deck.layout.cardHeight}; background:${deck.layout.backgroundColor}; color:${deck.layout.textColor}; border-color:${deck.layout.borderColor}; --accent-card:${deck.layout.accentColor}`}>
              {#each card.cornerLabels as label, cornerIndex}{#if card.cornerVisible[cornerIndex]}<span class={`mini-corner mini-corner-${cornerIndex}`}>{label}</span>{/if}{/each}
              {#if card.imageDataUrl && deck.layout.imageMode !== 'none'}<div class:full-image={deck.layout.imageMode === 'full'} class="mini-image"><img src={card.imageDataUrl} alt="" /></div>{/if}
              <div class="mini-content"><small>{card.subtitle || 'Tipo'}</small><strong>{card.title || 'Sin título'}</strong>{#if deck.layout.showDescription}<p>{card.description}</p>{/if}</div>
              {#if deck.layout.showStats}<div class="mini-stats">{#each card.stats.slice(0, 2) as stat}<span>{stat.value} {stat.label}</span>{/each}</div>{/if}
            </div>
    ...
    <aside class="panel editor-side">
      {#if selectedCard}
        <div class="panel-header"><div><p class="section-kicker">Edita en la carta</p><h2>{selectedCard.title}</h2></div></div>
        <div class="print-card-wrap">
          <div class={`print-card style-${deck.layout.style}`} style={`aspect-ratio:${deck.layout.cardWidth}/${deck.layout.cardHeight}; background:${deck.layout.backgroundColor}; color:${deck.layout.textColor}; border-color:${deck.layout.borderColor}; --accent-card:${deck.layout.accentColor}`}>
            {#each selectedCard.cornerLabels as label, index}{#if selectedCard.cornerVisible[index]}<input class={`corner corner-${index}`} maxlength="4" aria-label={cornerNames[index]} bind:value={selectedCard.cornerLabels[index]} on:change={() => persist('Esquina actualizada.')} />{/if}{/each}
            {#if deck.layout.imageMode !== 'none'}<label class:full-image={deck.layout.imageMode === 'full'} class="card-image inline-image-editor" title="Cambiar imagen">{#if selectedCard.imageDataUrl}<img src={selectedCard.imageDataUrl} alt={`Imagen de ${selectedCard.title}`} />{:else}<span>Haz clic para añadir imagen</span>{/if}<input type="file" accept="image/*" on:change={(event) => handleImageUpload(event, selectedCard)} /></label>{/if}
            <div class="card-content inline-card-content"><input class="inline-title" aria-label="Título" bind:value={selectedCard.title} on:change={() => persist('Título actualizado.')} /><input class="inline-subtitle" aria-label="Subtítulo" bind:value={selectedCard.subtitle} on:change={() => persist('Subtítulo actualizado.')} />{#if deck.layout.showDescription}<textarea class="inline-description" aria-label="Descripción" bind:value={selectedCard.description} on:change={() => persist('Descripción actualizada.')}></textarea>{/if}</div>
...
<footer class="site-footer"><div class="container-wide footer-inner"><p><strong>Card Creator</strong> · editor visual de cartas imprimibles.</p><p>Miniaturas con previsualización, inputs transparentes y PDF compatible.</p></div></footer>
